...
if($search=='detail_search'){
	...
	$total_count=$db->counter($web_img,$search_sql,'*');
}else{
	$total_count=$db->counter($web_img,$search_sql,'*');
}
...
$list_num=$web_img_list;
$rowset=new Pager($total_count,$list_num,$page);
$from_record=$rowset->_offset();
$query='select * from '.$web_img.$search_sql.$order_sql.' limit '.$from_record.','.$list_num;
$result=$db->query($query);
while($list=$db->fetch_array($result)){
...
$e_page_list=$rowset->link('img.php?en=en&class1='.$class1.'&class2='.$class2.'&class3='.$class3.'&search='.$search.'&c_title='.$c_title.'&page=');
