...
$query='select * from '.$web_parameter.' where type=3 and use_ok=1 order by no_order';
$result=$db->query($query);
while($list=$db->fetch_array($result)){
	if($list['maxsize']==100){
		$product_para100[]=$list;
	}
	$product_para[]=$list;
}
...
if($search=='detail_search'){
	...
	$total_count=$db->counter($web_product,$search_sql,'*');
}else{
	$total_count=$db->counter($web_product,$search_sql,'*');
}
...
$list_num=$web_product_list;
$rowset=new Pager($total_count,$list_num,$page);
$from_record=$rowset->_offset();
$query='select * from '.$web_product.$search_sql.$order_sql.' limit '.$from_record.','.$list_num;
$result=$db->query($query);
while($list=$db->fetch_array($result)){
	$url1_c='showproduct.php?id='.$list['id'];
	$url2_c='showproduct'.$list['id'].'.html';	
	$url1_e='showproduct.php?en=en&id='.$list['id'];
	$url2_e='showproduct'.$list['id'].'_en.html';
...
$e_page_list=$rowset->link('product.php?en=en&class1='.$class1.'&class2='.$class2.'&class3='.$class3.'&search='.$search.'&c_title='.$c_title.'&page=');
