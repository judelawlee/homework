...
switch($class1_info['module']){
	...
	break;	
	case 3:
		$table_name=$web_product;
		$preurl='../'.$class1_info['foldername'].'/showproduct';
	break;	
	case 4:
		$table_name=$web_download;
...
$total_count=$db->counter($table_name,$search_sql,'*');
require_once '../include/page.class.php';
$page=(int)$page;
if($page_input){
	$page=$page_input;
}
$list_num=$web_search_list;
$rowset=new Pager($total_count,$list_num,$page);
$from_record=$rowset->_offset();
$query='select * from '.$table_name.$search_sql.$order_sql.' limit '.$from_record.','.$list_num;
$result=$db->query($query);
while($list=$db->fetch_array($result)){
	$url1_c=$preurl.'.php?id='.$list['id'];
	$url2_c=$preurl.$list['id'].'.html';	
	$url1_e=$preurl.'.php?id='.$list['id'];
	$url2_e=$preurl.$list['id'].'.html';	
	$list['c_url']=$webhtml?$url2_c:$url1_c;
	$list['e_url']=$webhtml?$url2_e:$url1_e;
	$search_list[]=$list;
}
$c_page_list=$rowset->link('search.php?class1='.$class1.'&class2='.$class2.'&class3='.$class3.'&search='.$search.'&c_title='.$c_title.'&page=');
$e_page_list=$rowset->link('search.php?en=en&class1='.$class1.'&class2='.$class2.'&class3='.$class3.'&search='.$search.'&c_title='.$c_title.'&page=');
...
$nav_x['c_name']=$nav_x['c_name'].':搜索关键词--'.$c_title;
$nav_x['e_name']=$nav_x['e_name'].':keyword--'.$e_title;
if($en=='en'){
